plot4 <- function() {
        ...
        ## 3. finally it generates 4 plots (GAP vs. time, Vol vs. time, submetering vs. time and GRP vs. time)
        
        
        ## reads data from home directory
        ...
        ##Binds the new column to the dataset
        powerdata <- cbind(powerdata, FullTimeDate)
        
        ## changes the class on date and time columns
        powerdata$Date <- as.Date(powerdata$Date, format="%d/%m/%Y")
        powerdata$Time <- format(powerdata$Time, format="%H:%M:%S")
        ##changes class on remaining columns to numeric
